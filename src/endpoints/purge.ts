...
export async function purgeCloudflare(
  ...
  const info = (obj: any, msg: string) =>
    ctx.logger?.info?.call(ctx.logger, obj, msg) ?? console.log(msg, obj)
  const warn = (obj: any, msg: string) =>
    ctx.logger?.warn?.call(ctx.logger, obj, msg) ?? console.warn(msg, obj)
  const error = (obj: any, msg: string) =>
    ctx.logger?.error?.call(ctx.logger, obj, msg) ?? console.error(msg, obj)

  if (!zoneId || !apiToken) {
    warn(
      { correlationId: ctx.correlationId },
      'Cloudflare não configurado (zoneId/apiToken ausentes). Skip purge.',
    )
    return {
      ok: false,
      status: 0,
  ...
  try {
    ...
    error(
      { correlationId: ctx.correlationId, error: String(e) },
      'Falha de rede ao chamar Cloudflare',
    )
    throw e
  }
  ...
  info({ correlationId: ctx.correlationId, body: body }, 'Cloudflare purge concluído com sucesso')
